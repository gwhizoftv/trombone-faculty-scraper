...
echo "🚀 Launching $AGENT_COUNT codex agent tabs in isolated repos"
...
for i in $(seq 1 $AGENT_COUNT); do
  AGENT_REPO_DIR="$PARENT_DIR/${PROJECT_NAME}-codex$i"
  if [ ! -d "$AGENT_REPO_DIR" ]; then
    ...
    echo "   ./scripts/codex_agent_setup_isolated.sh" >&2
    exit 1
  fi
...
for i in $(seq 1 $AGENT_COUNT); do
  PROFILE_NAME="codex $i"
  AGENT_REPO_DIR="$PARENT_DIR/${PROJECT_NAME}-codex$i"
  
  # Updated command to run in the isolated agent's repo
  CMD_TO_RUN_IN_AGENT_SHELL="cd '$AGENT_REPO_DIR' && ./start-codex.sh"
