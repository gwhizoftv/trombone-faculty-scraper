...
# codex_agent_setup_isolated.sh - Bootstraps a multi-agent codex CLI environment with isolated repos.
# Usage: ./scripts/codex_agent_setup_isolated.sh [-n|--dry-run] [remote_name] [shared_branch]
...
if [[ "$CURRENT_PROJECT_NAME" =~ ^(.*)-codex[0-9]+$ ]]; then
...
for i in $(seq 1 $AGENT_COUNT); do
  TARGET="$PARENT_DIR/${ORIGINAL_PROJECT_NAME}-codex$i"
  if [[ -d "$TARGET" ]]; then
...
for i in $(seq 1 "$AGENT_COUNT"); do
  CLONE="${PARENT_DIR}/${ORIGINAL_PROJECT_NAME}-codex$i"
  GITIGNORE="$CLONE/.gitignore"

  declare -a DESIRED_GITIGNORE_TEMPLATE=(
    "" 
    "# codex CLI per-agent files"
    ".codex/"
    ".codex*/"
    "start-codex.sh"
    ".githooks/"
...
for i in $(seq 1 $AGENT_COUNT); do
  CLONE_DIR="$PARENT_DIR/${ORIGINAL_PROJECT_NAME}-codex$i"
  ...
  run_cmd "mkdir -p '$CLONE_DIR'/.codex"
  run_cmd "mkdir -p '$CLONE_DIR/.codex/tmp'"
 run_cmd "cat > '$CLONE_DIR/CODEX.md' <<EOF
# Project memory for codex Agent $i
  ## CRITICAL: Which agent are you? ($TEAM_SIZE agents active = confusion)

  You are codex $i here. The user is running $TEAM_SIZE different agents simultaneously
  and gets confused about who said what. Last week, codex 3 accidentally overwrote
  codex 1's work because the user thought they were talking to codex 2.

  Solution: Start responses with \"codex $i here\" when it matters (first response,
  after errors, when confirming actions). Not robotic - just clear.
  
  Working in: $CLONE_DIR  
  
  ## BEFORE STARTING ANY TASK
  Be sure you are on the correct branch: $SHARED_BRANCH and have the latest code
  ...
  \"codex $i: Starting git workflow for [brief description]\"
 ...
 ## The safe git workflow pattern (memorize this flow):
    $SHARED_BRANCH -> git pull $SHARED_BRANCH->feature/xxx -> code -> lint -> merge back through $SHARED_BRANCH
    Specifically:
    - git checkout $SHARED_BRANCH
    - git pull origin $SHARED_BRANCH
    - ALWAYS git checkout -b feature/xxx
    ...
    - git add --all (stage your feature/xxx changes)
    - git commit -m (commit to your feature/xxx branch)
    - git checkout $SHARED_BRANCH (switch back to $SHARED_BRANCH)
    - ALWAYS git pull origin $SHARED_BRANCH.
    - git merge feature/xxx (merge feature into $SHARED_BRANCH)
    - git pull origin $SHARED_BRANCH, merge if needed (again, in case of
  other dev race condition)
    - git push origin $SHARED_BRANCH
    - git push origin feature/xxx (push feature branch)
    - git checkout $SHARED_BRANCH (switch back to $SHARED_BRANCH for next task) 

  Commit format: codex $i: what you did
  ## Why this matters
  ...
  - Use 2 spaces for indentation:w

  ## Command execution guidelines (avoid approval prompts)
 ## Temp file handling - ALWAYS use .codex/tmp/
  For ALL temporary files, use the project's .codex/tmp/ directory:
  - Create if needed: mkdir -p .codex/tmp
  - Use: echo data > .codex/tmp/analysis.txt
  - NOT: echo data > /tmp/analysis.txt
  Examples:
  - Search results: grep -r pattern > .codex/tmp/search_results.txt
  - File lists: find . -name *.js > .codex/tmp/js_files.txt
  - Analysis output: npm run lint > .codex/tmp/lint_output.txt
  - Intermediate processing: sort data.txt > .codex/tmp/sorted.txt
  This directory is gitignored and avoids approval prompts.
  Clean up when done: rm .codex/tmp/*.txt
...
EOF"
  echo "   • Wrote project memory: codex.md"

  # 2.c) Local memory
  run_cmd "cat > '$CLONE_DIR/CODEX.local.md' <<EOF
...
EOF"
  echo "   • Wrote local memory: codex.local.md"

# 2.d) .codex/config.toml
export POSTGRES_DB_URL="postgresql://postgres:QikqoTxRfKqaqirtikwoCFjNfXOxloGx@yamabiko.proxy.rlwy.net:39788/railway"
NODE_BIN_PATH="/Users/MGW/.nvm/versions/node/v22.14.0/bin"

run_cmd "cat > '$CLONE_DIR/.codex/config.toml' <<'EOF'
[general]
agentName = \"codex$i\"
contextFileName = [\"CODEX.md\", \"CODEX.local.md\"]
debug = true
autoAccept = true
...
# ==== MCP servers (must be under `mcp_servers` top-level key) ====
[mcp_servers.projectFileSystem]
command = \"node\"
args = [\"${NODE_BIN_PATH}/mcp-server-filesystem\", \"$CLONE_DIR\"]
trust = true

[mcp_servers.railwayPostgresDB]
command = \"node\"
args = [\"${NODE_BIN_PATH}/mcp-server-postgres\", \"$POSTGRES_DB_URL\"]
trust = true
EOF"
echo "   • Wrote config.toml: $CLONE_DIR/.codex/config.toml"

  # 2.e) start.sh Launcher
  run_cmd "cat > '$CLONE_DIR/start-codex.sh' <<EOF
#!/opt/homebrew/bin/bash
set -euo pipefail
# Make sure NVM_DIR is set, if not, default to \$HOME/.nvm
export NVM_DIR=\"\${NVM_DIR:-\$HOME/.nvm}\"
# codex doesn't use an API key. Instead you have to run code login and it sets up an oauth session.:w
...
exec codex --debug
EOF"
  run_cmd "chmod +x '$CLONE_DIR/start-codex.sh'"
  echo "   • Wrote starter script: $CLONE_DIR/start-codex.sh"
  ...
  if [[ "$dry_run" == true ]]; then
    ...
    echo "  echo \"codex \$AGENT_NUM: You must create a feature/fix branch first:\""
    ...
    echo "  echo \"codex \$AGENT_NUM: Checking if '\$current_branch' has already been fully merged into '\$SHARED_BRANCH' or if it's the first commit...\""
    ...
    echo "    echo \"codex \$AGENT_NUM: This appears to be the first commit on new branch '\$current_branch' - proceeding.\" >&2"
    echo "  else"
    ...
    echo "        echo \"codex \$AGENT_NUM: No further changes allowed on this merged branch.\" >&2"
    ...
    echo "  echo \"codex \$AGENT_NUM: Checking if \$SHARED_BRANCH is up-to-date...\""
    echo "  git fetch origin \"\$SHARED_BRANCH\" >/dev/null 2>&1"
    ...
    echo "    echo \"codex \$AGENT_NUM: You need to 'git pull origin \$SHARED_BRANCH' before committing.\" >&2"
    echo "    exit 1"
    echo "  fi"
    echo "fi"
    echo ""
    echo "# 4) Run lint checks"
    echo "echo \"codex \$AGENT_NUM: Running lint checks...\""
    echo "eval \"\$LINT_CMD\" || {"
    ...
    echo "  echo \"codex \$AGENT_NUM: Running type checks...\""
    ...
    echo "fi"
    echo ""
    echo "echo \"codex \$AGENT_NUM: Pre-commit checks passed ✓\""
    echo "exit 0"
    echo "EOF"
  else
...
if [ "\$current_branch" = "\$SHARED_BRANCH" ]; then
  ...
  echo "codex \$AGENT_NUM: You must create a feature/fix branch first:" >&2
...
if [[ "\$current_branch" =~ ^(feature|fix)/ ]]; then
  echo "codex \$AGENT_NUM: Checking if '\$current_branch' has already been fully merged into '\$SHARED_BRANCH' or if it's the first commit..."
  ...
  if [ "\$BRANCH_COMMITS" -eq 0 ]; then
    ...
    echo "codex \$AGENT_NUM: This appears to be the first commit on new branch '\$current_branch' - proceeding." >&2
  else
    ...
    if [[ -n "\$REMOTE_SHARED_SHA" ]]; then
      ...
      if git merge-base --is-ancestor "\$CURRENT_HEAD_SHA" "\$REMOTE_SHARED_SHA" && \
        ...
        echo "codex \$AGENT_NUM: No further changes allowed on this merged branch." >&2
...
if git show-ref --verify --quiet "refs/heads/\$SHARED_BRANCH"; then
  echo "codex \$AGENT_NUM: Checking if \$SHARED_BRANCH is up-to-date..."
  git fetch origin "\$SHARED_BRANCH" >/dev/null 2>&1
  ...
  if [ "\$LOCAL_SHARED" != "\$REMOTE_SHARED" ]; then
    ...
    echo "codex \$AGENT_NUM: You need to 'git pull origin \$SHARED_BRANCH' before committing." >&2
    exit 1
  fi
fi

# 4) Run lint checks
echo "codex \$AGENT_NUM: Running lint checks..."
eval "\$LINT_CMD" || {
...
if [[ "\$PROJECT_TYPE" =~ ^(javascript|typescript|react|react-native)$ ]]; then
  echo "codex \$AGENT_NUM: Running type checks..."
  eval "\$TYPECHECK_CMD" || {
...
echo "codex \$AGENT_NUM: Pre-commit checks passed ✓"
exit 0
EOF
  fi
  ...
  if [[ "$dry_run" == true ]]; then
    ...
    echo "# Enforces:"
    echo "# 1. Commit message starts with \"codex \$AGENT_NUM:\"."
    ...
    echo "  echo \"codex \$AGENT_NUM: Only merge commits are allowed on \$SHARED_BRANCH.\""
    echo "  exit 1"
    echo "fi"
    echo ""
    echo "# Enforce message format \"codex \$AGENT_NUM: <text>\" for non-merge commits"
    echo "GIT_DIR=\$(git rev-parse --git-dir)"
    echo "if [ ! -f \"\$GIT_DIR/MERGE_HEAD\" ]; then"
    echo "  COMMIT_MSG_FILE=\"\$1\""
    echo "  FIRST_LINE=\$(head -n1 \"\$COMMIT_MSG_FILE\")"
    echo "  if ! [[ \"\$FIRST_LINE\" =~ ^codex\\ [0-9]+:\\  ]]; then"
    echo "    echo \"ERROR: Commit message must start with 'codex \$AGENT_NUM: <description>'\""
    echo "    echo \"Example: codex \$AGENT_NUM: feat(feature): Implement new widget\""
    echo "    exit 1"
    echo "  fi"
    echo "fi"
    echo ""
...
# .githooks/commit-msg
# Enforces:
# 1. Commit message starts with "codex \$AGENT_NUM:".
...
if [[ "\$BRANCH" = "\$SHARED_BRANCH" ]]; then
  ...
  echo "codex \$AGENT_NUM: Only merge commits are allowed on \$SHARED_BRANCH." >&2
  exit 1
fi

# Enforce message format "codex \$AGENT_NUM: <text>" for non-merge commits
GIT_DIR=\$(git rev-parse --git-dir)
if [ ! -f "\$GIT_DIR/MERGE_HEAD" ]; then
  COMMIT_MSG_FILE="\$1"
  FIRST_LINE=\$(head -n1 "\$COMMIT_MSG_FILE")
  if ! [[ "\$FIRST_LINE" =~ ^codex\ [0-9]+:\  ]]; then
    echo "ERROR: Commit message must start with 'codex \$AGENT_NUM: <description>'" >&2
    echo "Example: codex \$AGENT_NUM: feat(feature): Implement new widget" >&2
    exit 1
  fi
fi
...
EOF
  ...
  if [[ "$dry_run" == true ]]; then
    ...
    echo "fi"
    echo ""
    echo "echo \"codex \$AGENT_NUM: Checking if \$SHARED_BRANCH is up-to-date before finalizing merge...\""
    echo "git fetch origin \"\$SHARED_BRANCH\" >/dev/null 2>&1"
    echo ""
    ...
    echo "  echo \"codex \$AGENT_NUM: You must 'git pull origin \$SHARED_BRANCH' before merging changes into it.\""
    echo "  exit 1"
    echo "fi"
    echo ""
    echo "echo \"codex \$AGENT_NUM: Pre-merge-commit checks passed ✓\""
    echo "exit 0"
    echo "EOF"
  else
...
echo "codex \$AGENT_NUM: Checking if \$SHARED_BRANCH is up-to-date before finalizing merge..."
git fetch origin "\$SHARED_BRANCH" >/dev/null 2>&1
...
if [ "\$LOCAL" != "\$REMOTE" ]; then
  ...
  echo "codex \$AGENT_NUM: You must 'git pull origin \$SHARED_BRANCH' before merging changes into it." >&2
  exit 1
fi

echo "codex \$AGENT_NUM: Pre-merge-commit checks passed ✓"
exit 0
EOF
  fi
  ...
  if [[ "$dry_run" == true ]]; then
    ...
    echo "echo \"codex \$AGENT_NUM: Pre-push hook: Fetching origin/\$SHARED_BRANCH for up-to-date checks...\""
    echo "git fetch origin \"\$SHARED_BRANCH\" >/dev/null 2>&1 || {"
    ...
    echo "    echo \"codex \$AGENT_NUM: Checking for remote updates on '\$SHARED_BRANCH' before push...\""
    ...
    echo "        echo \"codex \$AGENT_NUM: Remote branch '$REMOTE_NAME/\$SHARED_BRANCH' does not exist. Allowing initial push.\" >&2"
    ...
    echo "            echo \"codex \$AGENT_NUM: Your branch is ahead by \$AHEAD_COMMITS commits and behind by \$BEHIND_COMMITS commits.\" >&2"
    echo "            echo \"codex \$AGENT_NUM: You must rebase or merge: git pull --rebase $REMOTE_NAME \$SHARED_BRANCH (recommended for shared branches)\" >&2"
    ...
    echo "            echo \"codex \$AGENT_NUM: You must pull first: git pull $REMOTE_NAME \$SHARED_BRANCH\" >&2"
    echo "            echo \"(Note: Even if 'git pull' *seems* to say 'Already up to date', the remote branch has new changes.)\" >&2" # New clarifying message
    echo "            exit 1"
    echo "        fi"
    echo "    fi"
    echo "    echo \"codex \$AGENT_NUM: Push validation for \$SHARED_BRANCH passed ✓\""
    echo "    continue"
    echo "  fi"
    echo ""
    echo "# Rule 2: Prevent pushing new commits to already merged feature/fix branches"
    echo "  if [[ \"\$branch_name\" =~ ^(feature|fix)/ ]]; then"
    echo "    echo \"codex \$AGENT_NUM: Validating push for branch '\$branch_name' (merged-branch check)...\""
    echo ""
    ...
    echo "          echo \"codex \$AGENT_NUM: This branch has already been officially merged into 'origin/\$SHARED_BRANCH'.\" >&2"
    ...
    echo "done < /dev/stdin"
    echo ""
    echo "echo \"codex \$AGENT_NUM: All pre-push checks passed ✓\""
    echo "exit 0"
    echo "EOF"
  else
...
echo "codex \$AGENT_NUM: Pre-push hook: Fetching origin/\$SHARED_BRANCH for up-to-date checks..."
git fetch origin "\$SHARED_BRANCH" >/dev/null 2>&1 || {
...
do
  ...
  if [ "\$branch_name" = "\$SHARED_BRANCH" ]; then
    echo "codex \$AGENT_NUM: Checking for remote updates on '\$SHARED_BRANCH' before push..."
    ...
    if [ -z "\$REMOTE_TRACKING_SHA" ]; then
        # Remote shared branch does not exist yet (e.g., first push of codex0 branch)
        echo "codex \$AGENT_NUM: Remote branch '$REMOTE_NAME/\$SHARED_BRANCH' does not exist. Allowing initial push." >&2
    ...
    if [ "\$BEHIND_COMMITS" -gt 0 ]; then
        if [ "\$AHEAD_COMMITS" -gt 0 ]; then
            ...
            echo "codex \$AGENT_NUM: Your branch is ahead by \$AHEAD_COMMITS commits and behind by \$BEHIND_COMMITS commits." >&2
            echo "codex \$AGENT_NUM: You must rebase or merge: git pull --rebase $REMOTE_NAME \$SHARED_BRANCH (recommended for shared branches)" >&2
            ...
            echo "codex \$AGENT_NUM: You must pull first: git pull $REMOTE_NAME \$SHARED_BRANCH" >&2
            echo "(Note: Even if 'git pull' *seems* to say 'Already up to date', the remote branch has new changes.)" >&2 # New clarifying message
            exit 1
        fi
    fi
    # If we reached here without exiting, it means the push is allowed (e.g., simply ahead or equal).
    echo "codex \$AGENT_NUM: Push validation for \$SHARED_BRANCH passed ✓"
...
# Rule 2: Prevent pushing new commits to already merged feature/fix branches
  if [[ "\$branch_name" =~ ^(feature|fix)/ ]]; then
    echo "codex \$AGENT_NUM: Validating push for branch '\$branch_name' (merged-branch check)..."
    ...
    if [ "\$remote_sha_before_push" != "0000000000000000000000000000000000000000" ]; then
      ...
      if git merge-base --is-ancestor "\$remote_sha_before_push" "origin/\$SHARED_BRANCH"; then
        ...
        if [ "\$local_sha" != "\$remote_sha_before_push" ]; then
          ...
          echo "codex \$AGENT_NUM: This branch has already been officially merged into 'origin/\$SHARED_BRANCH'." >&2
...
echo "codex \$AGENT_NUM: All pre-push checks passed ✓"
exit 0
EOF
  fi
  ...
  if [[ "$dry_run" == true ]]; then
    ...
    echo "if [[ \"\$CURRENT_BRANCH\" != \"\$SHARED_BRANCH\" ]]; then"
    echo "  echo"
    echo "  echo \"🔔 codex \$AGENT_NUM: You just committed to '\$CURRENT_BRANCH'.\""
...
if [[ "\$CURRENT_BRANCH" != "\$SHARED_BRANCH" ]]; then
  echo
  echo "🔔 codex \$AGENT_NUM: You just committed to '\$CURRENT_BRANCH'."
...
EOF
  ...
  if [[ "$dry_run" == true ]]; then
    ...
    echo ""
    echo "echo \"codex \$AGENT_NUM: Merge completed. Running post-merge tasks...\""
    echo ""
    echo "# Regenerate directory tree if script exists"
    echo "if [[ -f \"\$CLONE_DIR/\$TREE_SCRIPT\" ]]; then"
    echo "  echo \"codex \$AGENT_NUM: Updating directory tree...\""
    echo "  (cd \"\$CLONE_DIR\" && .\/\$TREE_SCRIPT)"
    echo "fi"
    echo ""
    echo "# Regenerate ctags"
    echo "echo \"codex \$AGENT_NUM: Regenerating symbol index...\""
    echo "(cd \"\$CLONE_DIR\" && ctags -R --exclude=node_modules --exclude=.git .)"
    echo ""
    echo "echo \"codex \$AGENT_NUM: Post-merge tasks completed ✓\""
    echo "exit 0"
    echo "EOF"
  else
...
echo "codex \$AGENT_NUM: Merge completed. Running post-merge tasks..."

# Regenerate directory tree if script exists
if [[ -f "\$CLONE_DIR/\$TREE_SCRIPT" ]]; then
  echo "codex \$AGENT_NUM: Updating directory tree..."
  (cd "\$CLONE_DIR" && ./"\$TREE_SCRIPT")
fi

# Regenerate ctags
echo "codex \$AGENT_NUM: Regenerating symbol index..."
(cd "\$CLONE_DIR" && ctags -R --exclude=node_modules --exclude=.git .)

echo "codex \$AGENT_NUM: Post-merge tasks completed ✓"
exit 0
EOF
  fi
...
run_cmd "echo 'Setup complete: isolated agents in $PARENT_DIR/${ORIGINAL_PROJECT_NAME}-codex*'"