...
# gemini_agent_setup_isolated.sh - Bootstraps a multi-agent Gemini CLI environment with isolated repos.
# Usage: ./scripts/gemini_agent_setup_isolated.sh [-n|--dry-run] [remote_name] [shared_branch]
...
if [[ "$CURRENT_PROJECT_NAME" =~ ^(.*)-gemini[0-9]+$ ]]; then
...
for i in $(seq 1 $AGENT_COUNT); do
  TARGET="$PARENT_DIR/${ORIGINAL_PROJECT_NAME}-gemini$i"
  if [[ -d "$TARGET" ]]; then
...
for i in $(seq 1 "$AGENT_COUNT"); do
  CLONE="${PARENT_DIR}/${ORIGINAL_PROJECT_NAME}-gemini$i"
  GITIGNORE="$CLONE/.gitignore"

  declare -a DESIRED_GITIGNORE_TEMPLATE=(
    "" 
    "# Gemini CLI per-agent files"
    ".gemini/"
    ".gemini*/"
    "start-gemini.sh"
    ".githooks/"
...
for i in $(seq 1 $AGENT_COUNT); do
  CLONE_DIR="$PARENT_DIR/${ORIGINAL_PROJECT_NAME}-gemini$i"
  ...
  run_cmd "mkdir -p '$CLONE_DIR'/.gemini"
  run_cmd "mkdir -p '$CLONE_DIR/.gemini/tmp'"
 run_cmd "cat > '$CLONE_DIR/GEMINI.md' <<EOF
# Project memory for Gemini Agent $i
  ## CRITICAL: Which agent are you? ($TEAM_SIZE agents active = confusion)

  You are Gemini $i here. The user is running $TEAM_SIZE different agents simultaneously
  and gets confused about who said what. Last week, Gemini 3 accidentally overwrote
  Gemini 1's work because the user thought they were talking to Gemini 2.

  Solution: Start responses with \"Gemini $i here\" when it matters (first response,
  ...
  \"Gemini $i: Starting git workflow for [brief description]\"
 ...
 ## The safe git workflow pattern (memorize this flow):
  ...
  Commit format: Gemini $i: what you did
  ## Why this matters
 ...
 ## Temp file handling - ALWAYS use .gemini/tmp/
  For ALL temporary files, use the project's .gemini/tmp/ directory:
  - Create if needed: mkdir -p .gemini/tmp
  - Use: echo data > .gemini/tmp/analysis.txt
  - NOT: echo data > /tmp/analysis.txt
  Examples:
  - Search results: grep -r pattern > .gemini/tmp/search_results.txt
  - File lists: find . -name *.js > .gemini/tmp/js_files.txt
  - Analysis output: npm run lint > .gemini/tmp/lint_output.txt
  - Intermediate processing: sort data.txt > .gemini/tmp/sorted.txt
  This directory is gitignored and avoids approval prompts.
  Clean up when done: rm .gemini/tmp/*.txt
...
EOF"
  echo "   • Wrote project memory: GEMINI.md"

  # 2.c) Local memory
  run_cmd "cat > '$CLONE_DIR/GEMINI.local.md' <<EOF
...
EOF"
  echo "   • Wrote local memory: GEMINI.local.md"

  # 2.d) .gemini/settings.json
  ...
  run_cmd "cat > '$CLONE_DIR/.gemini/settings.json' <<EOF
  {
    \"contextFileName\": [\"GEMINI.md\", \"GEMINI.local.md\"],
    \"debug\": true,
    \"autoAccept\": true,
    ...
    \"mcpServers\": {
      \"projectFileSystem\": {
        \"command\": \"node\",
        \"args\": [
          \"${NODE_BIN_PATH}/mcp-server-filesystem\",
          \"$CLONE_DIR\"
        ],
        \"trust\": true
      },
      \"railwayPostgresDB\": {
        \"command\": \"node\",
        \"args\": [
          \"${NODE_BIN_PATH}/mcp-server-postgres\",
          \"$POSTGRES_DB_URL\"
        ],
        \"trust\": true
      }
...
EOF"
  echo "   • Wrote settings.json: $CLONE_DIR/.gemini/settings.json"

  # 2.e) start.sh Launcher
  run_cmd "cat > '$CLONE_DIR/start-gemini.sh' <<EOF
#!/opt/homebrew/bin/bash
set -euo pipefail
# Make sure NVM_DIR is set, if not, default to \$HOME/.nvm
export NVM_DIR=\"\${NVM_DIR:-\$HOME/.nvm}\"
export GEMINI_API_KEY=\"AIzaSyCt6Tm5mapRff_e9fFc9hnVPfTA9pwAbfY\"
...
exec gemini --debug
EOF"
  run_cmd "chmod +x '$CLONE_DIR/start-gemini.sh'"
  echo "   • Wrote starter script: $CLONE_DIR/start-gemini.sh"
  ...
  if [[ "$dry_run" == true ]]; then
    ...
    echo "  echo \"Gemini \$AGENT_NUM: You must create a feature/fix branch first:\""
    ...
    echo "  echo \"Gemini \$AGENT_NUM: Checking if '\$current_branch' has already been fully merged into '\$SHARED_BRANCH' or if it's the first commit...\""
    ...
    echo "    echo \"Gemini \$AGENT_NUM: This appears to be the first commit on new branch '\$current_branch' - proceeding.\" >&2"
    echo "  else"
    ...
    echo "        echo \"Gemini \$AGENT_NUM: No further changes allowed on this merged branch.\" >&2"
    ...
    echo "  echo \"Gemini \$AGENT_NUM: Checking if \$SHARED_BRANCH is up-to-date...\""
    echo "  git fetch origin \"\$SHARED_BRANCH\" >/dev/null 2>&1"
    ...
    echo "    echo \"Gemini \$AGENT_NUM: You need to 'git pull origin \$SHARED_BRANCH' before committing.\" >&2"
    echo "    exit 1"
    echo "  fi"
    echo "fi"
    echo ""
    echo "# 4) Run lint checks"
    echo "echo \"Gemini \$AGENT_NUM: Running lint checks...\""
    echo "eval \"\$LINT_CMD\" || {"
    ...
    echo "  echo \"Gemini \$AGENT_NUM: Running type checks...\""
    ...
    echo "fi"
    echo ""
    echo "echo \"Gemini \$AGENT_NUM: Pre-commit checks passed ✓\""
    echo "exit 0"
    echo "EOF"
  else
...
if [ "\$current_branch" = "\$SHARED_BRANCH" ]; then
  ...
  echo "Gemini \$AGENT_NUM: You must create a feature/fix branch first:" >&2
...
if [[ "\$current_branch" =~ ^(feature|fix)/ ]]; then
  echo "Gemini \$AGENT_NUM: Checking if '\$current_branch' has already been fully merged into '\$SHARED_BRANCH' or if it's the first commit..."
  ...
  if [ "\$BRANCH_COMMITS" -eq 0 ]; then
    ...
    echo "Gemini \$AGENT_NUM: This appears to be the first commit on new branch '\$current_branch' - proceeding." >&2
  else
    ...
    if [[ -n "\$REMOTE_SHARED_SHA" ]]; then
      ...
      if git merge-base --is-ancestor "\$CURRENT_HEAD_SHA" "\$REMOTE_SHARED_SHA" && \
        ...
        echo "Gemini \$AGENT_NUM: No further changes allowed on this merged branch." >&2
...
if git show-ref --verify --quiet "refs/heads/\$SHARED_BRANCH"; then
  echo "Gemini \$AGENT_NUM: Checking if \$SHARED_BRANCH is up-to-date..."
  git fetch origin "\$SHARED_BRANCH" >/dev/null 2>&1
  ...
  if [ "\$LOCAL_SHARED" != "\$REMOTE_SHARED" ]; then
    ...
    echo "Gemini \$AGENT_NUM: You need to 'git pull origin \$SHARED_BRANCH' before committing." >&2
    exit 1
  fi
fi

# 4) Run lint checks
echo "Gemini \$AGENT_NUM: Running lint checks..."
eval "\$LINT_CMD" || {
...
if [[ "\$PROJECT_TYPE" =~ ^(javascript|typescript|react|react-native)$ ]]; then
  echo "Gemini \$AGENT_NUM: Running type checks..."
  eval "\$TYPECHECK_CMD" || {
...
echo "Gemini \$AGENT_NUM: Pre-commit checks passed ✓"
exit 0
EOF
  fi
  ...
  if [[ "$dry_run" == true ]]; then
    ...
    echo "# Enforces:"
    echo "# 1. Commit message starts with \"Gemini \$AGENT_NUM:\"."
    ...
    echo "  echo \"Gemini \$AGENT_NUM: Only merge commits are allowed on \$SHARED_BRANCH.\""
    echo "  exit 1"
    echo "fi"
    echo ""
    echo "# Enforce message format \"Gemini \$AGENT_NUM: <text>\" for non-merge commits"
    echo "GIT_DIR=\$(git rev-parse --git-dir)"
    echo "if [ ! -f \"\$GIT_DIR/MERGE_HEAD\" ]; then"
    echo "  COMMIT_MSG_FILE=\"\$1\""
    echo "  FIRST_LINE=\$(head -n1 \"\$COMMIT_MSG_FILE\")"
    echo "  if ! [[ \"\$FIRST_LINE\" =~ ^Gemini\\ [0-9]+:\\  ]]; then"
    echo "    echo \"ERROR: Commit message must start with 'Gemini \$AGENT_NUM: <description>'\""
    echo "    echo \"Example: Gemini \$AGENT_NUM: feat(feature): Implement new widget\""
    echo "    exit 1"
    echo "  fi"
    echo "fi"
    echo ""
...
# .githooks/commit-msg
# Enforces:
# 1. Commit message starts with "Gemini \$AGENT_NUM:".
...
if [[ "\$BRANCH" = "\$SHARED_BRANCH" ]]; then
  ...
  echo "Gemini \$AGENT_NUM: Only merge commits are allowed on \$SHARED_BRANCH." >&2
  exit 1
fi

# Enforce message format "Gemini \$AGENT_NUM: <text>" for non-merge commits
GIT_DIR=\$(git rev-parse --git-dir)
if [ ! -f "\$GIT_DIR/MERGE_HEAD" ]; then
  COMMIT_MSG_FILE="\$1"
  FIRST_LINE=\$(head -n1 "\$COMMIT_MSG_FILE")
  if ! [[ "\$FIRST_LINE" =~ ^Gemini\ [0-9]+:\  ]]; then
    echo "ERROR: Commit message must start with 'Gemini \$AGENT_NUM: <description>'" >&2
    echo "Example: Gemini \$AGENT_NUM: feat(feature): Implement new widget" >&2
    exit 1
  fi
fi
...
EOF
  ...
  if [[ "$dry_run" == true ]]; then
    ...
    echo "fi"
    echo ""
    echo "echo \"Gemini \$AGENT_NUM: Checking if \$SHARED_BRANCH is up-to-date before finalizing merge...\""
    echo "git fetch origin \"\$SHARED_BRANCH\" >/dev/null 2>&1"
    echo ""
    ...
    echo "  echo \"Gemini \$AGENT_NUM: You must 'git pull origin \$SHARED_BRANCH' before merging changes into it.\""
    echo "  exit 1"
    echo "fi"
    echo ""
    echo "echo \"Gemini \$AGENT_NUM: Pre-merge-commit checks passed ✓\""
    echo "exit 0"
    echo "EOF"
  else
...
echo "Gemini \$AGENT_NUM: Checking if \$SHARED_BRANCH is up-to-date before finalizing merge..."
git fetch origin "\$SHARED_BRANCH" >/dev/null 2>&1
...
if [ "\$LOCAL" != "\$REMOTE" ]; then
  ...
  echo "Gemini \$AGENT_NUM: You must 'git pull origin \$SHARED_BRANCH' before merging changes into it." >&2
  exit 1
fi

echo "Gemini \$AGENT_NUM: Pre-merge-commit checks passed ✓"
exit 0
EOF
  fi
  ...
  if [[ "$dry_run" == true ]]; then
    ...
    echo "echo \"Gemini \$AGENT_NUM: Pre-push hook: Fetching origin/\$SHARED_BRANCH for up-to-date checks...\""
    echo "git fetch origin \"\$SHARED_BRANCH\" >/dev/null 2>&1 || {"
    ...
    echo "    echo \"Gemini \$AGENT_NUM: Checking for remote updates on '\$SHARED_BRANCH' before push...\""
    ...
    echo "        echo \"Gemini \$AGENT_NUM: Remote branch '$REMOTE_NAME/\$SHARED_BRANCH' does not exist. Allowing initial push.\" >&2"
    ...
    echo "            echo \"Gemini \$AGENT_NUM: Your branch is ahead by \$AHEAD_COMMITS commits and behind by \$BEHIND_COMMITS commits.\" >&2"
    echo "            echo \"Gemini \$AGENT_NUM: You must rebase or merge: git pull --rebase $REMOTE_NAME \$SHARED_BRANCH (recommended for shared branches)\" >&2"
    ...
    echo "            echo \"Gemini \$AGENT_NUM: You must pull first: git pull $REMOTE_NAME \$SHARED_BRANCH\" >&2"
    echo "            echo \"(Note: Even if 'git pull' *seems* to say 'Already up to date', the remote branch has new changes.)\" >&2" # New clarifying message
    echo "            exit 1"
    echo "        fi"
    echo "    fi"
    echo "    echo \"Gemini \$AGENT_NUM: Push validation for \$SHARED_BRANCH passed ✓\""
    echo "    continue"
    echo "  fi"
    echo ""
    echo "# Rule 2: Prevent pushing new commits to already merged feature/fix branches"
    echo "  if [[ \"\$branch_name\" =~ ^(feature|fix)/ ]]; then"
    echo "    echo \"Gemini \$AGENT_NUM: Validating push for branch '\$branch_name' (merged-branch check)...\""
    echo ""
    ...
    echo "          echo \"Gemini \$AGENT_NUM: This branch has already been officially merged into 'origin/\$SHARED_BRANCH'.\" >&2"
    ...
    echo "done < /dev/stdin"
    echo ""
    echo "echo \"Gemini \$AGENT_NUM: All pre-push checks passed ✓\""
    echo "exit 0"
    echo "EOF"
  else
...
echo "Gemini \$AGENT_NUM: Pre-push hook: Fetching origin/\$SHARED_BRANCH for up-to-date checks..."
git fetch origin "\$SHARED_BRANCH" >/dev/null 2>&1 || {
...
do
  ...
  if [ "\$branch_name" = "\$SHARED_BRANCH" ]; then
    echo "Gemini \$AGENT_NUM: Checking for remote updates on '\$SHARED_BRANCH' before push..."
    ...
    if [ -z "\$REMOTE_TRACKING_SHA" ]; then
        # Remote shared branch does not exist yet (e.g., first push of gemini0 branch)
        echo "Gemini \$AGENT_NUM: Remote branch '$REMOTE_NAME/\$SHARED_BRANCH' does not exist. Allowing initial push." >&2
    ...
    if [ "\$BEHIND_COMMITS" -gt 0 ]; then
        if [ "\$AHEAD_COMMITS" -gt 0 ]; then
            ...
            echo "Gemini \$AGENT_NUM: Your branch is ahead by \$AHEAD_COMMITS commits and behind by \$BEHIND_COMMITS commits." >&2
            echo "Gemini \$AGENT_NUM: You must rebase or merge: git pull --rebase $REMOTE_NAME \$SHARED_BRANCH (recommended for shared branches)" >&2
            ...
            echo "Gemini \$AGENT_NUM: You must pull first: git pull $REMOTE_NAME \$SHARED_BRANCH" >&2
            echo "(Note: Even if 'git pull' *seems* to say 'Already up to date', the remote branch has new changes.)" >&2 # New clarifying message
            exit 1
        fi
    fi
    # If we reached here without exiting, it means the push is allowed (e.g., simply ahead or equal).
    echo "Gemini \$AGENT_NUM: Push validation for \$SHARED_BRANCH passed ✓"
...
# Rule 2: Prevent pushing new commits to already merged feature/fix branches
  if [[ "\$branch_name" =~ ^(feature|fix)/ ]]; then
    echo "Gemini \$AGENT_NUM: Validating push for branch '\$branch_name' (merged-branch check)..."
    ...
    if [ "\$remote_sha_before_push" != "0000000000000000000000000000000000000000" ]; then
      ...
      if git merge-base --is-ancestor "\$remote_sha_before_push" "origin/\$SHARED_BRANCH"; then
        ...
        if [ "\$local_sha" != "\$remote_sha_before_push" ]; then
          ...
          echo "Gemini \$AGENT_NUM: This branch has already been officially merged into 'origin/\$SHARED_BRANCH'." >&2
...
echo "Gemini \$AGENT_NUM: All pre-push checks passed ✓"
exit 0
EOF
  fi
  ...
  if [[ "$dry_run" == true ]]; then
    ...
    echo "if [[ \"\$CURRENT_BRANCH\" != \"\$SHARED_BRANCH\" ]]; then"
    echo "  echo"
    echo "  echo \"🔔 Gemini \$AGENT_NUM: You just committed to '\$CURRENT_BRANCH'.\""
...
if [[ "\$CURRENT_BRANCH" != "\$SHARED_BRANCH" ]]; then
  echo
  echo "🔔 Gemini \$AGENT_NUM: You just committed to '\$CURRENT_BRANCH'."
...
EOF
  ...
  if [[ "$dry_run" == true ]]; then
    ...
    echo ""
    echo "echo \"Gemini \$AGENT_NUM: Merge completed. Running post-merge tasks...\""
    echo ""
    echo "# Regenerate directory tree if script exists"
    echo "if [[ -f \"\$CLONE_DIR/\$TREE_SCRIPT\" ]]; then"
    echo "  echo \"Gemini \$AGENT_NUM: Updating directory tree...\""
    echo "  (cd \"\$CLONE_DIR\" && .\/\$TREE_SCRIPT)"
    echo "fi"
    echo ""
    echo "# Regenerate ctags"
    echo "echo \"Gemini \$AGENT_NUM: Regenerating symbol index...\""
    echo "(cd \"\$CLONE_DIR\" && ctags -R --exclude=node_modules --exclude=.git .)"
    echo ""
    echo "echo \"Gemini \$AGENT_NUM: Post-merge tasks completed ✓\""
    echo "exit 0"
    echo "EOF"
  else
...
echo "Gemini \$AGENT_NUM: Merge completed. Running post-merge tasks..."

# Regenerate directory tree if script exists
if [[ -f "\$CLONE_DIR/\$TREE_SCRIPT" ]]; then
  echo "Gemini \$AGENT_NUM: Updating directory tree..."
  (cd "\$CLONE_DIR" && ./"\$TREE_SCRIPT")
fi

# Regenerate ctags
echo "Gemini \$AGENT_NUM: Regenerating symbol index..."
(cd "\$CLONE_DIR" && ctags -R --exclude=node_modules --exclude=.git .)

echo "Gemini \$AGENT_NUM: Post-merge tasks completed ✓"
exit 0
EOF
  fi
...
run_cmd "echo 'Setup complete: isolated agents in $PARENT_DIR/${ORIGINAL_PROJECT_NAME}-gemini*'"